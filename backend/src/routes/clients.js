...
router.post('/', authMiddleware, sellerOrAdmin, (req, res) => {
  const { name, cpf, birth_date, whatsapp, cep, street, number, complement, neighborhood, city, state, due_day, plan_id, seller_id } = req.body;
  ...
  const info = db.prepare(`INSERT INTO clients (name,cpf,birth_date,whatsapp,cep,street,number,complement,neighborhood,city,state,due_day,plan_id,seller_id)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(name, cleanCpf, birth_date||null, whatsapp||null, cep||null, street||null, number||null, complement||null, neighborhood||null, city||null, state||null, due_day||null, plan_id||null, sid);
...
router.put('/:id', authMiddleware, (req, res) => {
  ...
  const { name, birth_date, whatsapp, cep, street, number, complement, neighborhood, city, state, due_day, status, plan_id } = req.body;
  const newStatus = req.user.role === 'admin' && status ? status : client.status;

  db.prepare(`UPDATE clients SET name=?, birth_date=?, whatsapp=?, cep=?, street=?, number=?, complement=?, neighborhood=?, city=?, state=?, due_day=?, status=?, plan_id=? WHERE id=?`)
    .run(name||client.name, birth_date||client.birth_date, whatsapp||client.whatsapp, cep||client.cep, street||client.street, number||client.number, complement||client.complement, neighborhood||client.neighborhood, city||client.city, state||client.state, due_day||client.due_day, newStatus, plan_id||client.plan_id, req.params.id);
